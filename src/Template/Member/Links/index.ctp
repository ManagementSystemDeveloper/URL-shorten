...
<?php foreach ($links as $link) : ?>
    ...
    if (get_option('link_info_member', 'yes') == 'yes') {
        $link_info_member = "<i class='fa fa-bar-chart'></i> <a href='{$link->permalink()}/info' target='_blank'>" . __('Stats') . "</a> - ";
    }
    ?>
    ...
    <div class="box box-solid">
        <div class="box-body">
            <h4><a href="<?= $link->permalink() ?>" target="_blank" rel="nofollow noopener noreferrer"><span
            ...
            <p class="text-muted">
                <small><?= $link_info_member ?><i
                            ...
                            rel="nofollow noopener noreferrer"><?= strtoupper(parse_url(
                            $link->url,
                            PHP_URL_HOST
                        )); ?></a></small>
            </p>
